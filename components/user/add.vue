...
const categories = ref(["Admin", "User", "Guest"]);
...
const addUser = async () => {
  if (email.value && selectedCategory.value) {
    ...
    try {
      // Replace the below commented code with your actual API call
      ...
      toastMessage("success", 200, `User added successfully`);
      // if (response.ok) {
      // Reset fields if necessary
      email.value = "";
      selectedCategory.value = "";
      // } else {
      //   toastMessage("error", 500, `Error: ${error.message}`);
      // }
    } catch (error) {
      toastMessage("error", 500, `Error: ${error.message}`);
    }
